...
class ToolDiscovery:
    ...
    def _definitions(self) -> list[ToolDefinition]:
        return [
            ...
            self._tool("documents.search", "Search existing documents", "documents", ToolCapability.DOCUMENT,
                       self._documents_search, {"query": "string", "limit": "integer"}, required=("query",)),
            self._tool("import.file", "Import a file through the enterprise import engine", "import", ToolCapability.CONNECTOR,
            ...
            self._tool("agents.status", "Read existing agent status", "agents", ToolCapability.WORKFLOW_CORE, self._agents_status),
            self._tool("agents.plan.create", "Create a plan with the existing Agent Planner", "agents", ToolCapability.WORKFLOW_CORE,
                       self._agent_plan, {"goal": "string", "workspace_id": "string"}, required=("goal",), risk=ToolRiskLevel.HIGH),
            self._tool("jobs.list", "List jobs from the native job queue", "jobs", ToolCapability.WORKFLOW_CORE,
                       self._jobs_list, {"status": "string", "kind": "string"}),
            self._tool("jobs.cancel", "Cancel a job in the native job queue", "jobs", ToolCapability.WORKFLOW_CORE,
                       self._job_cancel, {"job_id": "string"}, required=("job_id",), risk=ToolRiskLevel.HIGH),
            self._tool("notifications.list", "List native notifications", "notifications", ToolCapability.SYSTEM_CORE,
                       self._notifications_list, {"limit": "integer", "unread_only": "boolean"}),
            self._tool("notifications.send", "Create a native notification", "notifications", ToolCapability.SYSTEM_CORE,
            ...
            self._tool("settings.show", "Show existing embedding and store settings", "settings", ToolCapability.SYSTEM_CORE,
                       self._settings_show),
            self._tool("voice.status", "Read native voice status", "voice", ToolCapability.SYSTEM_CORE, self._voice_status),
            self._tool("voice.parse", "Parse a German voice command", "voice", ToolCapability.SYSTEM_CORE,
                       self._voice_parse, {"text": "string"}, required=("text",)),
            self._tool("updates.check", "Check the existing local update manifest", "updates", ToolCapability.SYSTEM_CORE,
                       self._updates_check, {"current_version": "string"}),
            self._tool("github.status", "Check availability of the existing GitHub connector", "github", ToolCapability.CONNECTOR,
                       self._github_status),
            self._tool("filesystem.list", "List files below the project root", "filesystem", ToolCapability.FILE,
                       self._filesystem_list, {"path": "string", "limit": "integer"}),
            self._tool("filesystem.read", "Read a UTF-8 text file below the project root", "filesystem", ToolCapability.FILE,
