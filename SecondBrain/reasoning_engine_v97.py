...
def write_reasoning_map(vault: Path = VAULT) -> Path:
    ...
    for p in iter_notes(vault):
        text = read(p)
        sig = signals(text)
        link_count = len(links(text))
        impact = sig["risk"] * 3 + sig["decision"] * 2 + link_count
    ...
    for row in sorted(rows, key=lambda x: x["impact"], reverse=True)[:80]:
        sig = row["signals"]
        interpretation = (
            "Review priorisieren"
            if sig["risk"]
            else "Entscheidungskontext prüfen"
            if sig["decision"]
            else "Kontextknoten"
        )

        lines.append(
            f"| {row['impact']} | [[{row['stem']}]] | {sig['risk']} | "
            f"{sig['decision']} | {row['link_count']} | {interpretation} |"
        )
