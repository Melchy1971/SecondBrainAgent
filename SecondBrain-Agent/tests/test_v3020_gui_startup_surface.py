...
def test_gui_commands_include_legacy_and_current_aliases():
    assert {"gui", "gui-start", "gui-open", "desktop-gui", "desktop16-gui", "gui-status", "gui-doctor", "gui-shortcuts"}.issubset(GUI_COMMANDS)


def test_gui_status_is_safe_when_stopped(tmp_path):
    payload = gui_status(tmp_path)
    assert payload["status"] in {"stopped", "running"}
...
def test_shortcut_manifest_documents_manual_start_paths(tmp_path):
    ...
    assert any(item["name"] == "Jarvis GUI" for item in payload["desktop_shortcuts"])
    assert "python launcher.py gui-open" in payload["manual_start"]
